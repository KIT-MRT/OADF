...
extern const char UNICARBrake_LibIdent[];
const char UNICARBrake_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_UNICARBrake *rtm)
{
...
static void
UNICARBrake_DeclQuants (void *MP)
{
    rtModel_UNICARBrake *rtm = (rtModel_UNICARBrake *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(UNICARBrake_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(UNICARBrake_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, UNICARBrake_DictDefines[i].Name);
	}
...
/*
 * UNICARBrake_SetParams() will be invoked indirectly by the generated
 * model C code each time UNICARBrake_New() is called.
 */
void
UNICARBrake_SetParams (rtModel_UNICARBrake *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
{
    rtModel_UNICARBrake *rtm = (rtModel_UNICARBrake *)MP;
    ExternalInputs_UNICARBrake  *in  = (ExternalInputs_UNICARBrake *) rtmGetU(rtm);
...
static void *
UNICARBrake_New (struct tInfos *Inf, tBrakeCfgIF *CfgIF, const char *KindKey)
{
    rtModel_UNICARBrake *rtm;
    ...
    MatSupp_ResetQuants(UNICARBrake_DictDefines);
...
static int
UNICARBrake_Calc (void *MP, tBrakeIF *IF, double dt)
{
    rtModel_UNICARBrake *rtm = (rtModel_UNICARBrake *)MP;
    int osCount=0;
    ExternalInputs_UNICARBrake  *in  = (ExternalInputs_UNICARBrake *) rtmGetU(rtm);
    ExternalOutputs_UNICARBrake *out = (ExternalOutputs_UNICARBrake *)rtmGetY(rtm);
...
UNICARBrake_Register (void)
{
    ...
    m.Brake.DeclQuants		= UNICARBrake_DeclQuants;
    m.Brake.New			= UNICARBrake_New;
    m.Brake.Calc		= UNICARBrake_Calc;
    m.Brake.Delete		= UNICARBrake_Delete;
